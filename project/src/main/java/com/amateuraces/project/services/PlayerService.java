...
public class PlayerService {
    ...
    public List<Tournament> getRegisteredTournaments(String playerId) {
        ...
        // Assuming Player entity has a method to get the tournaments the player is registered for
        return player.getRegisteredTournaments();
    }

    // Method to get the match schedule for a player
    public List<Match> viewMatchSchedule(String playerId) {
        // Find the player by ID
        Player player = playerRepository.findById(playerId)
                .orElseThrow(() -> new EntityNotFoundException("No player with that ID"));

        // Assuming Player entity has a method to get the player's match schedule
        return player.getMatchSchedule();
    }
